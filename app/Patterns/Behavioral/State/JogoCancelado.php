...
/**
 * Padrão State — Estado: Cancelado (Terminal)
 *
 * Nenhuma transição é permitida a partir deste estado.
 */
class JogoCancelado implements JogoStateInterface
{
    private function negar(string $acao): never
    {
        throw new \LogicException("Não é possível {$acao} um jogo cancelado.");
    }
    ...
    public function cancelar(Jogo $jogo): void        { throw new \LogicException("O jogo já está cancelado."); }
    public function encerrar(Jogo $jogo): void        { $this->negar('encerrar'); }
}
