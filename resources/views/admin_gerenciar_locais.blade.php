            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ !$local->ativo ? 'opacity-50 bg-light' : '' }}">
                            <td class="ps-4">
                                <span class="fw-bold {{ !$local->ativo ? 'text-decoration-line-through text-muted' : '' }}">
                                    {{ $local->nome }}
                                </span>
                                @if(!$local->ativo)
                            ...
                            <td class="text-center">
                                <div class="d-flex gap-2 justify-content-center">
                                    {{-- Editar: Desabilitado se estiver inativo --}}
                                    ...
                                    <form action="{{ route('alternar_status_local') }}" method="POST">
                                        ...
                                        @if($local->ativo)
                                            <button type="submit" class="btn btn-sm btn-outline-warning" local="Desativar">
                                                <i class="bi bi-eye-slash"></i>
                                            </button>
                                        @else
                                            <button type="submit" class="btn btn-sm btn-success" local="Reativar">
                                                <i class="bi bi-eye"></i>
                                            </button>
                                        @endif
                                    </form>
